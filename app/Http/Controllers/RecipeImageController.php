...
class RecipeImageController extends BaseController
{
    ...
    {
        $images = Image::whereHas('recipe', function ($query) use ($recipe) {
            $query->where('id',  $recipe->id);
            return $query;
        })->pimp()->paginate();
        ImageResource::collection($images);
        return $this->sendResponse(compact('recipe', 'images'), "All Records");
    }
     ...
     * @group RecipeImage
     */
    public function store(Recipe $recipe, Image $image, CreateRequest $request)
    {
     ...
     * @group RecipeImage
     */
    public function destroy(Recipe $recipe, Image $image, Request $request)
    {
        $recipe->images()->find($image->id)->delete();
